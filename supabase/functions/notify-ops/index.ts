...
serve(async (req) => {
  ...
  try {
    ...
    if (RESEND_API_KEY) {
      ...
      const htmlBody = guidedSessionRequest
        ...
        <html>
          <body style="font-family:Arial,sans-serif;max-width:500px;margin:0 auto;padding:20px;">
            <h2 style="color:#7B51D3;">Guided Session Request</h2>
            <p>${name} (${email}) has completed the Tixie orientation and is requesting a guided 1-hour training session.</p>
            <p style="margin-top:16px;"><strong>Quiz Score:</strong> ${score}%</p>
            <p style="color:#666;margin-top:16px;">Please reach out to schedule their session.</p>
          </body>
        </html>`
        : `<!DOCTYPE html>
      ...
      console.warn("[NOTIFY-OPS] RESEND_API_KEY not set — email not sent");
    }
